...
import { GearFill, Trash, Tools, Dash,Plus } from "react-bootstrap-icons";
...
const Orden =()=>{
  ...
  // const data=useSelector(state=> state);
    ...
    // console.log(data)
    useEffect(()=>{ //ver ruta getOrderCart
...
const handleClick = (e, id, name) => { //Debe eliminar el producto del carrito
  e.preventDefault();
  Swal.fire({
    ...
    function quant(e, type, quantity){ //Debería modificarse para incrementar el numero de productos
      // e.preventDefault();
      let sum=quantity
        ...
        <tbody className="scroll">
          {data.products.map((item, index) => (
            <tr className="my-auto" key={index}>
              ...
              <td>$ {item.price}</td>
              <td>{item.quantity}{' '} <Dash onClick={(e)=>{ let type= 'decrement'; quant(e, type, item.quantity)}}/><Plus onClick={(e)=>{let type= 'increment'; quant(e, type, item.quantity)}}/></td>
              <td className="p-2">
                <Button
                  color="default"
